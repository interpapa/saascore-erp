              ...
              >
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-full bg-slate-100 dark:bg-slate-800 flex items-center justify-center text-slate-600 dark:text-slate-400 font-bold group-hover:bg-primary/10 group-hover:text-primary transition-colors">
                    {item[columns[0].field]?.charAt(0)}
                  </div>
                  <div>
                    <p className="font-semibold text-foreground">{item[columns[0].field]}</p>
                    <p className="text-xs text-slate-600 dark:text-slate-400 font-medium">{item[columns[1].field]}</p>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-bold text-foreground">
                    {columns[2].format === 'currency' ? '$' : ''}{item[columns[2].field]}
                  </p>
                  {columns[3] && (
