...
-- 2. TABLA: RELACIÃ“N USUARIO-TENANT (RBAC & SECURITY)
...
-- 4. TABLA: CATÃLOGO DE ÃTEMS (PRODUCTOS / SERVICIOS / SUSCRIPCIONES)
...
-- 8. TABLA: LÃNEAS DE ASIENTOS CONTABLES (PARTIDA DOBLE)
...
-- 9. TABLA: LOGS DE AUDITORÃA INMUTABLES (WORM - ISO 27001)
...
-- ==============================================================================
-- ÃNDICES PARCIALES DE ALTO RENDIMIENTO Y LECTURAS ACTIVAS
-- ==============================================================================
...
-- ==============================================================================
-- RPC ATÃ“MICO EN POSTGRESQL PARA CONCURRENCIA DE STOCK (FOR UPDATE)
-- ==============================================================================
...
-- ============================================================
-- MigraciÃ³n: Agregar columnas faltantes a la tabla appointments
...
-- Agregar columna 'description' (TEXT, descripciÃ³n de la cita)
...
-- (estas probablemente ya estÃ¡n: id, tenant_id, employee_id, start_time, end_time, status, created_at, updated_at)
...
-- ==============================================================================
-- MIGRACIÃ“N DE SEGURIDAD Y CONCURRENCIA: Decremento AtÃ³mico de Inventario
-- Archivo: supabase/migrations/20260806_atomic_stock.sql
-- ==============================================================================

-- Esta funciÃ³n ejecuta una resta atÃ³mica con bloqueo de fila en PostgreSQL
-- para evitar Race Conditions / Lost Updates en ventas simultÃ¡neas.
...
DECLARE
  v_current_stock INT;
BEGIN
  -- 1. Intentar seleccionar la fila con bloqueo exclusivo para actualizaciÃ³n
  SELECT stock INTO v_current_stock
  FROM items
  WHERE id = p_item_id AND tenant_id = p_tenant_id
  FOR UPDATE;
  ...
  -- 3. Si el stock es NULL, es un servicio o producto digital (sin inventario fÃ­sico)
  IF v_current_stock IS NULL THEN
    RETURN QUERY SELECT TRUE, 0, NULL::TEXT;
    RETURN;
  END IF;
  ...
  -- 5. Decremento atÃ³mico ininterrumpible
  UPDATE items
  SET stock = stock - p_quantity
  WHERE id = p_item_id AND tenant_id = p_tenant_id;
...
-- ==============================================================================
-- MIGRACIÃ“N DE INTEGRIDAD REFERENCIAL: Soft Deletes (deleted_at)
-- Archivo: supabase/migrations/20260806_soft_deletes.sql
-- ==============================================================================

-- AÃ±adir columna deleted_at a las tablas principales
...
-- Ãndices parciales para optimizar consultas de lectura activas
